...
extension SchemaMigrations {
    static let applyHardinessZoneText: @Sendable (Database) throws -> Void = { db in
        try db.execute(
            sql: """
        ...
        try db.execute(
            sql: """
                ...
                SELECT id, name, notes, archived, latitude, longitude,
                    CAST(hardiness_zone AS TEXT),
                    last_frost_month, last_frost_day,
                    first_frost_month, first_frost_day
                FROM property
                """
        )
